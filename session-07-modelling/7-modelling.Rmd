...
```{r, echo= FALSE}
reg_1 <- lm(life_expectancy ~ gdp + population + alcohol, data = life_expec_dat)

reg_2 <- lm(life_expectancy ~ gdp * population + alcohol, data = life_expec_dat)

gdp_squared <- (life_expec_dat$gdp)^2
pop_log <- log(life_expec_dat$population)
reg_3 <- lm(life_expectancy ~ gdp + gdp_squared + pop_log, data = life_expec_dat)
```
:::
...
```{r, echo= FALSE}
summary(multiv_model_out[[1]]) # life_expectancy ~ gdp
summary(multiv_model_out[[4]]) # life_expectancy ~ bmi
summary(multiv_model_out[[31]]) # life_expectancy ~ gdp + percentage_expenditure + alcohol + bmi + hiv_aids
```
:::
